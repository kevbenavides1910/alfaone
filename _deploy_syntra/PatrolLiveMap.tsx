...
function FitBounds({
  ...
  const points = useMemo(() => {
    ...
    const fromTrails = trails.flatMap((t) => trailLatLngs(t));
...
export function PatrolLiveMap({
  ...
  const allPoints = [
    ...
    ...visibleTrails.flatMap((t) => trailLatLngs(t)),
  ];
  ...
  return (
    <>
      ...
      >
        ...
        />
        <MapResizeFix />
        <FitBounds devices={devices} trails={visibleTrails} fitKey={fitKey} />
        {visibleTrails.map((trail) => (
          <Polyline
            key={trail.deviceId}
            positions={trailLatLngs(trail)}
            pathOptions={{
              color: trail.color,
              weight: 4,
              opacity: 0.75,
