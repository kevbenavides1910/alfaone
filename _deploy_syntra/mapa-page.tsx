...
export default function RecorridosMapaPage() {
  ...
  const mapFitKey = useMemo(
    () =>
      [deviceId, routeId, historyDesde, historyHasta, showTrails, refitNonce].join("|"),
    [deviceId, routeId, historyDesde, historyHasta, showTrails, refitNonce],
  );
  ...
  const historyQueryKey = useMemo(
    ...
    [deviceId, routeId, historyDesde, historyHasta],
  );
  ...
  const { data, isLoading, isFetching, refetch, dataUpdatedAt } = useQuery<LiveResponse>({
    ...
    queryFn: () => {
      ...
      return fetch(`/api/admin/patrol/tracking/live${q ? `?${q}` : ""}`).then((r) => r.json());
    },
  ...
  const {
    ...
    queryFn: () => {
      ...
      if (deviceId) params.set("deviceId", deviceId);
      if (routeId) params.set("routeId", routeId);
      return fetch(`/api/admin/patrol/tracking/history?${params}`).then((r) => r.json());
    },
    enabled: showTrails,
  });
      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Posicion actual y recorrido GPS (hasta 2 meses). Actualizacion en vivo cada 10 segundos.
          </p>
        </div>
        ...
        <CardContent className="space-y-3">
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            <p className="text-xs text-muted-foreground">
              ...
              {showTrails && !loadingHistory ? (
                <> · {totalHistoryPoints} puntos en el periodo</>
              ) : null}
            </p>
          </div>
        </CardContent>
        ...
        <div className="space-y-4">
          ...
          <Card className="border-dashed">
            <CardContent className="pt-4 text-xs text-muted-foreground space-y-1">
              <p>Lineas de color: recorrido GPS en el rango de fechas seleccionado.</p>
              <p>Flechas sobre la linea: direccion del movimiento; pase el cursor para ver la hora.</p>
