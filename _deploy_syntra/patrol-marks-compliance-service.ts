...
function markTimeInCostaRica(markedAt: Date): string {
...
function markWithinWindow(markedAt: Date, startTime: string, endTime: string): boolean {
  const t = markTimeInCostaRica(markedAt);
...
function markMatchesPointWindow(
  ...
  if (mark.imei !== deviceImei) return false;
  if (isoDateInCostaRica(mark.markedAt) !== fecha) return false;
