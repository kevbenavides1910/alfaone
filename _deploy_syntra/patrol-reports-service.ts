...
import { updateDeviceLastGps, pruneDeviceGpsTracks } from "@/modules/syntra/services/patrol-live-tracking-service";
...
export async function savePatrolMark(input: {
  ...
  return prisma.patrolMark.create({
    data: {
      ...
      markedAt: parseTimestamp(input.markedAt),
...
export async function savePatrolGpsTrack(input: {
  ...
  void pruneDeviceGpsTracks(input.deviceId);
  return track;
}
