...
export async function addAuthorizedPhone(routeId: string, assetId: string) {
  const route = await prisma.patrolRoute.findUnique({ where: { id: routeId } });
  ...
  const row = await prisma.patrolRoutePhone.upsert({
    where: { routeId_assetId: { routeId, assetId } },
    create: { routeId, assetId, isPrimary: false },
    update: route.positionId ? {} : { isPrimary: false },
  });

  if (asset.currentPosition) {
...
export async function removeAuthorizedPhone(routeId: string, phoneRowId: string) {
  ...
  if (row.isPrimary && route?.positionId) throw new Error("CANNOT_REMOVE_PRIMARY");
