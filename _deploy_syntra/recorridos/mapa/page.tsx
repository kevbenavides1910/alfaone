...
import { RefreshCw, Radio, Route, Smartphone } from "lucide-react";
...
export default function RecorridosMapaPage() {
  ...
  const trails: MapTrail[] = useMemo(() => {
    ...
    return historyData.data.trails.map((t, i) => ({
      ...
      points: t.points.map((p) => [p.latitude, p.longitude] as [number, number]),
          ...
          <CardContent className="p-0">
            <div className="h-[min(70vh,560px)]">
              {isLoading ? (
                ...
                <PatrolLiveMap devices={allDevices} trails={trails} showTrails={showTrails} />
              )}
            </div>
          </CardContent>
        </Card>
