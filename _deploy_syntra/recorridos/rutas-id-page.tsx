...
export default function RecorridosRutaDetailPage({
  ...
  const primaryPhone = route.phones.find((p) => p.isPrimary);
  const additionalPhones = route.phones.filter((p) => !p.isPrimary);
        ...
        <CardContent className="space-y-4">
          ...
          {additionalPhones.length > 0 && (
            <div className="space-y-2">
              <p className="text-xs font-medium text-muted-foreground uppercase tracking-wide">
                Teléfonos de apoyo (cualquier inventario)
              </p>
          ...
          {additionalPhones.length === 0 && (
            <p className="text-sm text-muted-foreground">
              Puede autorizar celulares de cualquier contrato o puesto para que ayuden a cumplir
              esta ruta.
            </p>
          )}
        </CardContent>
