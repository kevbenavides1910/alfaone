...
sudo nano /etc/presupuestos-alfa/health-alert.env

# Prueba
HEALTH_ALERT_ENV=/etc/presupuestos-alfa/health-alert.env \
...
| `scripts/setup-health-alert-email.sh` | Crea `/etc/presupuestos-alfa/health-alert.env` |
...
Cron instalado en `/etc/cron.d/presupuestos-alfa`. Logs en `/var/log/presupuestos-alfa/`.
...
sudo nano /etc/presupuestos-alfa/backup-remote.env
#   BACKUP_REMOTE_HOST=IP_O_HOSTNAME_DEL_OTRO_SERVIDOR
#   BACKUP_REMOTE_USER=usuario_con_escritura
#   BACKUP_REMOTE_PATH=/backups/presupuestos-alfa/postgres
...
# o solo sync: BACKUP_REMOTE_ENV=/etc/presupuestos-alfa/backup-remote.env bash scripts/postgres-backup-remote-sync.sh
