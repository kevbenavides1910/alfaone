#!/usr/bin/env bash
# Crea la estructura de archivos en el disco grande (/mnt/storage) para Presupuestos-Alfa.
...
STORAGE_ROOT="${APP_DATA_HOST:-/mnt/storage/apps/presupuestos-alfa}"
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --group)
      ...
      ;;
    -h|--help)
      echo "Uso: $0 [--root /mnt/storage/apps/presupuestos-alfa] [--user soporte-ti]"
      exit 0
      ;;
    *)
...
if ! mountpoint -q /mnt/storage 2>/dev/null; then
  echo "AVISO: /mnt/storage no está montado. Verifique el RAID/HDD antes de continuar." >&2
fi
