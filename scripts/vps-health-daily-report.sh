...
oracle_backup="(sin log)"
oracle_log="$(ls -1t "$LOG_DIR"/backup-to-oracle_*.log 2>/dev/null | head -1 || true)"
if [ -n "$oracle_log" ]; then
  oracle_backup="$(basename "$oracle_log") — $(grep '^\* Final:' "$oracle_log" | tail -1 | sed 's/^\* Final: //')"
fi
