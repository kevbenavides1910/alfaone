...
export default function ExpensesPageClient({ initialExpenses }: { initialExpenses: Expense[] }) {
  ...
  const expenses = useMemo(() => {
    ...
    const byReference = bySearch.filter((e) => {
      if (!filterReference) return true;
      const q = filterReference.toLowerCase().trim();
      const originRef = [e.origin?.name, e.referenceNumber].filter(Boolean).join(" · ").toLowerCase();
      const originRefSpaced = [e.origin?.name, e.referenceNumber].filter(Boolean).join(" ").toLowerCase();
      return (
        e.referenceNumber?.toLowerCase().includes(q) ||
        e.origin?.name?.toLowerCase().includes(q) ||
        originRef.includes(q) ||
        originRefSpaced.includes(q)
      );
    });
    if (filterDistribution === "all") return byReference;
    return byReference.filter((e) => expenseDistributionKind(e) === filterDistribution);
  }, [data?.data, search, filterReference, filterDistribution]);
        ...
        <Card>
          <CardContent className="p-4">
            <div className="flex flex-wrap gap-3">
              ...
              <div className="relative w-56 min-w-48">
                <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-slate-400" />
                <Input
                  placeholder="Origen / N° referencia u OC..."
