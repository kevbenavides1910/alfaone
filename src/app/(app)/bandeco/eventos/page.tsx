...
export default function BandecoEventosPage() {
  ...
  const { data, isLoading } = useQuery<{ data: EventoRow[] }>({
    queryKey: ["bandeco-eventos"],
    queryFn: () => fetch("/api/bandeco/eventos").then((r) => r.json()),
  });

  const mutation = useMutation({
    mutationFn: async () => {
      const r = await fetch("/api/bandeco/eventos", {
        method: "POST",
    ...
    onSuccess: () => {
      ...
      setMotivo("");
      setInforme("");
      void qc.invalidateQueries({ queryKey: ["bandeco-eventos"] });
    },
  ...
  return (
    <div className="p-4 md:p-6 space-y-6">
      <div>
        ...
        <p className="text-sm text-slate-500">Equivalente a la hoja EVENTOS y GENERARBITACORA.</p>
      </div>
      ...
      <Card className="max-w-2xl">
        ...
        <CardContent className="space-y-3">
          ...
          <Input placeholder="Motivo (ej. Pila media vacía)" value={motivo} onChange={(e) => setMotivo(e.target.value)} />
          <Textarea placeholder="Informe detallado" value={informe} onChange={(e) => setInforme(e.target.value)} rows={5} />
          <div className="flex gap-2">
            <Button onClick={() => mutation.mutate()} disabled={!finca || !informe || mutation.isPending}>
              Registrar evento
            </Button>
