...
import type { BandecoConsultaResult } from "@/modules/bandeco/services/consulta";
...
export default function BandecoConsultaPage() {
  ...
  const { data, isLoading, isError } = useQuery<{ data: BandecoConsultaResult }>({
    queryKey: ["bandeco-consulta", searchCode],
    queryFn: () =>
      fetch(`/api/bandeco/consulta/${searchCode}`, { credentials: "same-origin" }).then(async (r) => {
