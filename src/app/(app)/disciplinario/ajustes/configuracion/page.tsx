...
export default function DisciplinarioAjustesConfiguracionPage() {
  ...
  const readOnly = !hasPermission(session, "disciplinario.ajustes", "edit");
  ...
  useEffect(() => {
    ...
    setForm({
      ...
      smtpSecure:
        Number(d.smtpPort ?? 587) === 465
          ? true
          : Number(d.smtpPort ?? 587) === 587
            ? false
            : Boolean(d.smtpSecure),
          ...
          <CardContent className="space-y-4">
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <div className="space-y-1.5 sm:col-span-2">
                ...
                <input className="w-full rounded border px-3 py-2 text-sm" value={form.smtpHost} onChange={(e) => setForm((s) => ({ ...s, smtpHost: e.target.value }))} disabled={readOnly} placeholder="srv.grupocorporativoalfa.com" />
                <p className="text-xs text-slate-500">
                  Puerto 587 usa STARTTLS aunque no marque «SSL/TLS». Use{" "}
                  <span className="font-mono">srv.grupocorporativoalfa.com</span> o{" "}
                  <span className="font-mono">mail.seguridadalfa.com</span>.
                  No use <span className="font-mono">webmail.seguridadalfa.com</span> (es la web, no el servidor SMTP).
                </p>
              </div>
              <div className="space-y-1.5">
                ...
                <input
                  ...
                  onChange={(e) => {
                    ...
                    const p = Number(smtpPort || 0);
                    setForm((s) => ({
                      ...s,
                      smtpPort,
                      smtpSecure: p === 465 ? true : p === 587 ? false : s.smtpSecure,
                    }));
                  }}
                  disabled={readOnly}
                />
            ...
            <label className="inline-flex items-center gap-2 text-sm text-slate-700">
              <input
                ...
                disabled={readOnly || form.smtpPort === "587"}
              />
              Usar conexión segura (SSL/TLS)
            </label>
            {form.smtpPort === "587" && (
              <p className="text-xs text-amber-700">
                En puerto 587 la conexión ya usa STARTTLS automáticamente; no marque SSL/TLS (solo aplica al puerto 465).
              </p>
            )}
            <div className="space-y-1.5">
