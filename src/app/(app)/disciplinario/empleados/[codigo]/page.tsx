...
export default function EmployeeDetailPage({
  ...
  const canManage = canManageDisciplinary(session ?? null);
                    ...
                    <>
                      ...
                      {data.data.treatment && (
                        <>
                          ...
                          <div className="text-xs text-slate-500">
                            Ubic.: {data.data.ubicacion ?? "—"} · Suc.:{" "}
                            {data.data.sucursal ?? "—"}
                          </div>
                          <div className="text-xs text-slate-500">
                            Cobrado:{" "}
                            {data.data.treatment.cobradoDate
                              ? formatDate(data.data.treatment.cobradoDate)
                              : "—"}
                          </div>
                        </>
                      )}
                    ...
                    <tbody className="divide-y">
                      {data.data.closedCycles.map((c) => (
                        <tr key={c.id} className="hover:bg-muted/50">
                          ...
                          <td className="px-3 py-2">
                            {formatCycleClosureLabel(c.accion, c.accionRaw)}
                          </td>
                          <td className="px-3 py-2 text-right">
