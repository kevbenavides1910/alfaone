...
interface BatchRow {
  ...
  emailSummary?: {
    ...
    emailsPendingSendable: number;
    emailsPendingNoEmail: number;
  } | null;
}
...
export default function DisciplinarioImportPage() {
  ...
  const [batchEmailRetryId, setBatchEmailRetryId] = useState<string | null>(null);
  ...
  const canConfigureImport = hasPermission(session, "disciplinario.import", "admin");
  ...
  const { data: batchZonesEmail, isLoading: batchZonesLoading } = useQuery({
    ...
    enabled: canConfigureImport && Boolean(bulkPdfBatchId),
  });
  ...
  const sendPendingMarcasEmailsMutation = useMutation({
    ...
    onMutate: (batchId) => setBatchEmailRetryId(batchId),
    ...
    onError: (e: Error) => toast.error(e.message),
    onSettled: () => setBatchEmailRetryId(null),
  });
  ...
  if (!canImport) {
    return (
      <>
        <div className="p-6">
          <div className="text-rose-600">No tenés permiso para importar lotes disciplinarios.</div>
        </div>
      </>
    );
  }
        ...
        {marcasResult && (
          <Card>
            ...
            <CardContent className="space-y-3">
              ...
              {(marcasResult.emailsSkipped > (marcasResult.emailsSkippedNoEmail ?? 0) ||
                ...
                <div className="rounded border border-amber-200 bg-amber-50/80 p-3 text-sm text-amber-900 space-y-2">
                  <p>
                    {marcasResult.emailsSent === 0
                      ? "No se envió ningún correo en esta importación."
                      : "Quedaron apercibimientos sin correo enviado."}{" "}
                    Lo más habitual es que{" "}
                    <strong>SMTP no esté guardado</strong> o haya fallado el envío. Revise{" "}
                    <Link href="/disciplinario/ajustes/configuracion" className="text-blue-700 hover:underline">
                      Ajustes → Configuración
                    </Link>
                    {marcasResult.emailsSkippedNoEmail
                      ? ` · ${marcasResult.emailsSkippedNoEmail} empleado(s) no tienen correo en Empleados.`
                      : null}
                    {" · También puede reintentar desde el historial de importaciones."}
                  </p>
                  <Button
                    type="button"
                    size="sm"
        ...
        <Card>
          <CardHeader>
            ...
            <p className="text-xs text-slate-500 font-normal">
              ...
              importarlo otra vez. En lotes de marcas con correos pendientes puede usar{" "}
              <strong>Reintentar envío</strong>.
            </p>
          </CardHeader>
              ...
              <table className="w-full text-sm">
                <thead className="bg-muted/50 border-b text-xs uppercase text-slate-500">
                  <tr>
                    ...
                    <th className="px-3 py-2 text-right">Correos</th>
                  ...
                  {batchesData.data.map((b) => (
                    <tr key={b.id} className="hover:bg-muted/50">
                      ...
                      <td className="px-3 py-2 text-right text-xs tabular-nums">
                        {b.notes?.startsWith("import_marcas") && b.emailSummary ? (
                          <div className="space-y-0.5">
                            <div className="text-emerald-700">{b.emailSummary.emailsSent} enviados</div>
                            {b.emailSummary.emailsPending > 0 ? (
                              <div className="text-amber-700">
                                {b.emailSummary.emailsPending} pendiente
                                {b.emailSummary.emailsPending !== 1 ? "s" : ""}
                                {b.emailSummary.emailsPendingNoEmail > 0
                                  ? ` (${b.emailSummary.emailsPendingNoEmail} sin correo)`
                                  : null}
                              </div>
                            ) : (
                      ...
                      <td className="px-3 py-2 text-right">
                        <div className="flex flex-col items-end gap-1">
                          {b.notes?.startsWith("import_marcas") &&
                          (b.emailSummary?.emailsPendingSendable ?? 0) > 0 ? (
                            <Button
                              type="button"
                              variant="outline"
                              size="sm"
                              className="h-8 gap-1 text-blue-700 hover:text-blue-800 hover:bg-blue-50"
                              disabled={
                                sendPendingMarcasEmailsMutation.isPending || !smtpReady
                              }
                              title={
                                !smtpReady
                                  ? "Configure y guarde SMTP en Ajustes → Configuración"
                                  : undefined
                              }
                              onClick={() => sendPendingMarcasEmailsMutation.mutate(b.id)}
                            >
                              <Mail className="h-3.5 w-3.5" />
                              {batchEmailRetryId === b.id && sendPendingMarcasEmailsMutation.isPending
                                ? "Enviando…"
                                : `Reintentar envío (${b.emailSummary?.emailsPendingSendable})`}
                            </Button>
                          ) : null}
                          <Button
                            type="button"
