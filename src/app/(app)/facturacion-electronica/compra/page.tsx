...
export default function FacturaCompraListPage() {
  const { data: session } = useSession();
  const qc = useQueryClient();
  const { companyCode } = useFeCompany();
  const canEdit = hasPermission(session, "facturacionElectronica.compras", "edit");

  const listQ = useQuery({
    queryKey: ["fe-facturas-compra", companyCode],
    queryFn: async () => {
      const r = await fetch(feApiUrl("/api/fe/facturas-compra", companyCode));
      const j = await r.json();
      if (!r.ok) throw new Error(j.error?.message ?? "Error al listar");
      return j.data as { items: FeCompraRow[]; total: number };
    },
    enabled: hasPermission(session, "facturacionElectronica.compras", "view") && Boolean(companyCode),
  });

  const enviarM = useMutation({
    mutationFn: async (id: string) => {
      const r = await fetch(feApiUrl(`/api/fe/facturas-compra/${id}/enviar`, companyCode), { method: "POST" });
      const j = await r.json();
      if (!r.ok) throw new Error(j.error?.message ?? "Error al enviar");
      return j.data;
    },
    onSuccess: () => {
      toast.success("Factura de compra encolada para envío");
      void qc.invalidateQueries({ queryKey: ["fe-facturas-compra"] });
    },
  ...
  const estadoM = useMutation({
    mutationFn: async (id: string) => {
      const r = await fetch(feApiUrl(`/api/fe/facturas-compra/${id}/estado`, companyCode));
      const j = await r.json();
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h2 className="text-lg font-semibold">Facturas electrónicas de compra (FEC)</h2>
          <p className="text-sm text-muted-foreground">{listQ.data?.total ?? 0} documento(s)</p>
        </div>
        {canEdit && (
          <Button asChild>
            <Link href="/facturacion-electronica/compra/nueva">
      ...
      <Card>
        <CardContent className="p-0">
          {listQ.isLoading ? (
            ...
            <p className="p-4 text-sm text-muted-foreground">No hay facturas de compra registradas.</p>
          ) : (
                ...
                <tbody>
                  {listQ.data.items.map((row) => (
                    <tr key={row.id} className="border-b last:border-0 hover:bg-muted/20">
                      ...
                      <td className="px-4 py-2">{row.proveedorNombre}</td>
                            ...
                            {["ACEPTADA", "ACEPTADA_PARCIALMENTE"].includes(row.estado) && (
                              <>
                                <Button size="sm" variant="outline" asChild>
                                  <Link href={`/facturacion-electronica/compra/${row.id}/nota-credito`}>
                                    <FileMinus className="mr-1 h-3 w-3" />
                                    NC
                                  </Link>
                                </Button>
                                <Button size="sm" variant="outline" asChild>
                                  <Link href={`/facturacion-electronica/compra/${row.id}/nota-debito`}>
                            ...
                            {row.comprobante?.claveNumerica && (
                              <>
                                <Button size="sm" variant="ghost" asChild>
                                  <a href={feApiUrl(`/api/fe/facturas-compra/${row.id}/pdf`, companyCode)} download>
                                    <Download className="h-3 w-3" />
                                  </a>
                                </Button>
                                <Button size="sm" variant="ghost" asChild>
                                  <a href={feApiUrl(`/api/fe/facturas-compra/${row.id}/xml`, companyCode)} download>
