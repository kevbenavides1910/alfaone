...
export default function BandecoAperturasCierresPage() {
  ...
  const { data, isLoading } = useQuery<{ data: AycRow[] }>({
    queryKey: ["monitoreo-ayc"],
    queryFn: () => fetch("/api/monitoreo/aperturas-cierres").then((r) => r.json()),
  });

  const { data: pendientes } = useQuery<{ data: AycRow[] }>({
    queryKey: ["monitoreo-ayc-pendientes"],
    queryFn: () => fetch("/api/monitoreo/aperturas-cierres?pendientes=1").then((r) => r.json()),
  });

  const mutation = useMutation({
    mutationFn: async () => {
      const r = await fetch("/api/monitoreo/aperturas-cierres", {
        method: "POST",
    ...
    onSuccess: () => {
      ...
      setHoraApertura("");
      setHoraCierre("");
      void qc.invalidateQueries({ queryKey: ["monitoreo-ayc"] });
      void qc.invalidateQueries({ queryKey: ["monitoreo-ayc-pendientes"] });
    },
