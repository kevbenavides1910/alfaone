...
import { ArrowLeft, Plus, Pencil, Trash2, Clock, MapPin } from "lucide-react";
...
function groupPointDays(rows: { pointId: string; dayOfWeek: number }[]): PointDayAssignment[] {
...
function getPointIdsForDay(days: PointDayAssignment[], dayOfWeek: number): string[] {
  return days.find((d) => d.dayOfWeek === dayOfWeek)?.pointIds ?? [];
}

function setPointIdsForDay(
  form: { samePointsEveryDay: boolean; days: PointDayAssignment[] },
  dayOfWeek: number,
  pointIds: string[],
) {
  const other = form.days.filter((d) => d.dayOfWeek !== dayOfWeek);
  if (pointIds.length === 0) return { ...form, days: other };
  return {
    ...form,
    days: [...other, { dayOfWeek, pointIds }].sort((a, b) => a.dayOfWeek - b.dayOfWeek),
  };
}
...
export default function RecorridosRutaDetailPage({
  ...
  const pointDaysMeta =
    ...
    (route
      ? {
          samePointsEveryDay: route.samePointsEveryDay ?? true,
          days: groupPointDays(route.pointDays ?? []),
        }
      : null);
    ...
    mutationFn: async () => {
      ...
      const res = await fetch(`/api/admin/patrol/routes/${id}/point-days`, {
        ...
        body: JSON.stringify({
          ...
          days: pointDaysMeta.days,
        }),
      });
      if (!res.ok) {
        const j = await res.json().catch(() => ({}));
        throw new Error(j?.error?.message ?? "Error al guardar puntos por día");
      }
    },
    onSuccess: () => {
      toast.success("Plan de puntos actualizado");
  ...
  function togglePointForDay(dayOfWeek: number, pointId: string, checked: boolean) {
    if (!pointDaysMeta) return;
    const current = getPointIdsForDay(pointDaysMeta.days, dayOfWeek);
    const next = checked ? [...current, pointId] : current.filter((id) => id !== pointId);
    setPointDaysForm(setPointIdsForDay(pointDaysMeta, dayOfWeek, next));
  }
  ...
  if (isLoading || !route || !meta || !scheduleMeta || !pointDaysMeta) {
  ...
  return (
    <div className="p-6 md:p-8 max-w-6xl mx-auto w-full space-y-6">
      ...
      <Card>
        ...
        <CardContent className="space-y-4 max-w-3xl">
          <div>
            <Label htmlFor="route-point-mode">Plan de visitas</Label>
            <Select
              value={pointDaysMeta.samePointsEveryDay ? "same" : "weekly"}
              onValueChange={(value) =>
                setPointDaysForm({
                  ...pointDaysMeta,
                  samePointsEveryDay: value === "same",
                })
              }
            >
              <SelectTrigger id="route-point-mode" className="max-w-md">
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
                <SelectItem value="same">Mismos puntos todos los días</SelectItem>
                <SelectItem value="weekly">Puntos distintos según el día</SelectItem>
              </SelectContent>
            </Select>
            <p className="text-xs text-muted-foreground mt-1">
              {pointDaysMeta.samePointsEveryDay
                ? "La app móvil mostrará todos los puntos de la ruta cada día en que la ruta esté habilitada."
                : "Seleccione qué puntos deben visitarse en cada día. Los días sin puntos no generarán marcas esperadas."}
            </p>
          </div>
          ...
          {!pointDaysMeta.samePointsEveryDay && (
            <div className="space-y-4">
              {route.points.length === 0 ? (
                <p className="text-sm text-muted-foreground">
                  Agregue puntos de marca antes de definir el plan semanal.
                </p>
              ) : (
                DAY_LABELS.map((dayLabel, dayOfWeek) => {
                  const selectedIds = getPointIdsForDay(pointDaysMeta.days, dayOfWeek);
                  return (
                    <div key={dayOfWeek} className="rounded-md border px-3 py-3 space-y-2">
                      <div className="flex items-center justify-between gap-2">
                        <p className="text-sm font-medium">{dayLabel}</p>
                        <span className="text-xs text-muted-foreground">
                          {selectedIds.length} de {route.points.length} punto
                      ...
                      <div className="grid gap-2 sm:grid-cols-2">
                        {route.points.map((p) => {
                          ...
                          return (
                            <label
                              key={`${dayOfWeek}-${p.id}`}
                              className="flex items-center gap-2 text-sm rounded-md border border-border/60 px-2 py-2 cursor-pointer hover:bg-muted/40"
                            >
                              <input
                                type="checkbox"
                                checked={checked}
                                onChange={(e) => togglePointForDay(dayOfWeek, p.id, e.target.checked)}
                              />
          ...
          <Button
            ...
            disabled={savePointDays.isPending || (!pointDaysMeta.samePointsEveryDay && route.points.length === 0)}
          >
            Guardar plan de puntos
          </Button>
        </CardContent>
      </Card>
