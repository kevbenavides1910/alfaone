...
import { activacionSchema } from "@/modules/bandeco/validations/schemas";
import { registrarActivacion, listActivaciones } from "@/modules/bandeco/services/activaciones";

export async function GET(req: NextRequest) {
  const session = await getSession();
  if (!session) return unauthorized();
  if (!requirePermission(session, "bandeco.registros", "view")) return forbidden();
...
export async function POST(req: NextRequest) {
  const session = await getSession();
  if (!session) return unauthorized();
  if (!requirePermission(session, "bandeco.operacion", "edit")) return forbidden();
