...
import { ensureDefaultBillingRequirements } from "@/modules/presupuestos/business/contractBillingRequirementsDefaults";

export async function GET(req: NextRequest) {
  const session = await getSession();
  if (!session) return unauthorized();
  const { hasPermission } = await import("@/lib/permissions/check");
  if (!hasPermission(session, "alfa-one.contracts", "view")) return forbidden();
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    await ensureDefaultBillingRequirements(
      prisma,
      contract.id,
      { client: contract.client, notes: contract.notes, licitacionNo: contract.licitacionNo },
      { createdById: session.user.id }
    );
