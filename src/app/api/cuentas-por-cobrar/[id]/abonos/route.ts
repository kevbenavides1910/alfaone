...
import { cxcAbonoBodySchema } from "@/modules/presupuestos/validations/cuentas-por-cobrar.schema";
import {
  createCxcAbono,
...
export async function POST(req: NextRequest, { params }: Ctx) {
  ...
  try {
    ...
    const parsed = cxcAbonoBodySchema.safeParse(body);
    if (!parsed.success) return badRequest("Datos inválidos", parsed.error.flatten());

    const result = await createCxcAbono(prisma, id, parsed.data, session.user.id);
    if (!result.ok) {
    ...
    return serverError("Error al registrar abono", e);
  }
}
