...
import { updateCxcGestionSchema } from "@/modules/presupuestos/validations/cuentas-por-cobrar.schema";
import {
...
export async function PATCH(req: NextRequest, { params }: Ctx) {
  ...
  try {
    ...
    const parsed = updateCxcGestionSchema.safeParse(body);
    if (!parsed.success) return badRequest("Datos inválidos", parsed.error.flatten());

    const result = await updateCxcGestion(prisma, id, parsed.data);
    if (!result.ok) {
    ...
    return serverError("Error al guardar gestión de cobro", e);
  }
}
