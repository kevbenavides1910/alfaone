...
export async function PATCH(req: NextRequest, { params }: Ctx) {
  ...
  try {
    ...
    const editableWhenClosed = new Set([
      "invoiceNumber",
      "documentNumber",
    ...
    const touchedKeys = Object.entries(parsed.data)
      .filter(([, value]) => value !== undefined)
      .map(([key]) => key);
    const onlyClosedMetadata =
      touchedKeys.length > 0 && touchedKeys.every((key) => editableWhenClosed.has(key));

    if (isClosed && !onlyClosedMetadata) {
