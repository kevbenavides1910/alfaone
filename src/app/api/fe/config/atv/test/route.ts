...
import { testFeAtvSchema } from "@/modules/facturacion-electronica/validators/atv.schema";
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const parsed = testFeAtvSchema.safeParse(body);
    if (!parsed.success) return badRequest("Datos inválidos", parsed.error.flatten());

    const companyCode = resolveFeCompanyCodeFromSession(session, body.companyCode);
    const result = await controller.testAtv(companyCode, parsed.data);
    return ok(result);
  } catch (e) {
