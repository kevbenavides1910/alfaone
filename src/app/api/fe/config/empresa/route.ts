...
import { upsertFeEmpresaSchema } from "@/modules/facturacion-electronica/validators/empresa.schema";
...
export async function PUT(req: NextRequest) {
  ...
  try {
    ...
    const parsed = upsertFeEmpresaSchema.safeParse(body);
    if (!parsed.success) return badRequest("Datos inválidos", parsed.error.flatten());

    const companyCode = resolveFeCompanyCodeFromSession(session, body.companyCode);
    const empresa = await controller.upsertEmpresa(
      companyCode,
      { ...parsed.data, atvPasswordStg: body.atvPasswordStg?.trim() || undefined },
      session.user.id
    );
    return ok(empresa);
  } catch (e) {
