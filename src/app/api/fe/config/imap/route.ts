...
import { updateFeImapSchema } from "@/modules/facturacion-electronica/validators/imap.schema";
...
export async function PUT(req: NextRequest) {
  ...
  try {
    ...
    const parsed = updateFeImapSchema.safeParse(body);
    if (!parsed.success) return badRequest("Datos inválidos", parsed.error.flatten());

    const companyCode = resolveFeCompanyCodeFromSession(session, body.companyCode);
    const empresa = await controller.updateImap(companyCode, parsed.data, session.user.id);
    return ok(empresa);
  } catch (e) {
