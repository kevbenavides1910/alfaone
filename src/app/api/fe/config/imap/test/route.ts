...
import { testFeImapSchema } from "@/modules/facturacion-electronica/validators/imap.schema";
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const parsed = testFeImapSchema.safeParse(body);
    if (!parsed.success) return badRequest("Datos inválidos", parsed.error.flatten());

    const companyCode = resolveFeCompanyCodeFromSession(session, body.companyCode);
    const result = await controller.testImap(companyCode, parsed.data);
    return ok(result);
  } catch (e) {
