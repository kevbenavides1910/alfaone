...
import { updateFePuntoVentaSchema } from "@/modules/facturacion-electronica/validators/empresa.schema";
...
export async function PATCH(req: NextRequest, ctx: Ctx) {
  ...
  try {
    ...
    const parsed = updateFePuntoVentaSchema.safeParse(body);
    if (!parsed.success) return badRequest("Datos inválidos", parsed.error.flatten());

    const { id } = await ctx.params;
    const companyCode = resolveFeCompanyCodeFromSession(session, body.companyCode);
    const pv = await controller.updatePuntoVenta(companyCode, id, parsed.data, session.user.id);
    return ok(pv);
  } catch (e) {
...
export async function DELETE(req: NextRequest, ctx: Ctx) {
  ...
  try {
    ...
    await controller.deletePuntoVenta(companyCode, id, session.user.id);
    return noContent();
  } catch (e) {
    return mapFeErrorToResponse(e);
  }
