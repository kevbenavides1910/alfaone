...
import { createFeFacturaCompraSchema } from "@/modules/facturacion-electronica/validators/compra.schema";
...
export async function POST(req: NextRequest) {
  ...
  if (!hasPermission(session, "facturacionElectronica.compras", "edit")) return forbidden();
  try {
    const body = await req.json();
    const parsed = createFeFacturaCompraSchema.safeParse(body);
