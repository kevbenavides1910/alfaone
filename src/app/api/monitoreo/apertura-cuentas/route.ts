...
import { aperturaCuentaSchema } from "@/modules/monitoreo/validations/schemas";
import { listAperturaCuentas, createAperturaCuenta } from "@/modules/monitoreo/services/catalogs-service";

export async function GET(req: NextRequest) {
  const session = await getSession();
  if (!session) return unauthorized();
  if (!requirePermission(session, "monitoreo.mantenimientos", "view")) return forbidden();
  try {
...
export async function POST(req: NextRequest) {
  const session = await getSession();
  if (!session) return unauthorized();
  if (!requirePermission(session, "monitoreo.mantenimientos", "edit")) return forbidden();
  try {
