...
import { updateSigProcess, deleteSigProcess } from "@/modules/sig/services/catalogs";
...
export async function PATCH(req: NextRequest, { params }: Ctx) {
  ...
  try {
    ...
    const row = await updateSigProcess(id, {
    ...
    if (err) return err;
    return serverError("Error al actualizar proceso", e);
  }
}
...
export async function DELETE(_req: NextRequest, { params }: Ctx) {
  ...
  try {
    ...
    const err = clientError(e);
    if (err) return err;
    return serverError("Error al eliminar proceso", e);
  }
}
