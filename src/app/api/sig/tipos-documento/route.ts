...
import { listSigDocumentTypes, createSigDocumentType } from "@/modules/sig/services/catalogs";
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const rows = await listSigDocumentTypes(includeInactive);
    return ok(
      rows.map((r) => ({
        ...r,
    ...
    return serverError("Error al listar tipos documentales", e);
  }
}
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const row = await createSigDocumentType({
      code: body.code,
      name: body.name,
    ...
    return serverError("Error al crear tipo documental", e);
  }
}
