    ...
    <aside className="w-64 text-white flex flex-col min-h-screen" style={{ backgroundColor: sidebarBg }}>
      ...
      <nav className="flex-1 px-4 py-4 space-y-1">
        {SIDEBAR_NAV_ITEMS.filter((item) => {
          ...
          if (item.adminOnly && !isPlatformAdmin(session)) {
            return false;
          }
          ...
          if (item.href === "/contracts") {
            return hasPermission(session, "alfa-one.contracts", "view");
          }
      ...
      <div className="px-4 py-4 border-t border-white/10">
        <div className="flex items-center gap-3 px-3 py-2">
          ...
          <div className="overflow-hidden">
            ...
            <div className="text-xs text-slate-400 truncate">{session?.user?.roleCode ?? session?.user?.role}</div>
          </div>
        </div>
      </div>
    </aside>
