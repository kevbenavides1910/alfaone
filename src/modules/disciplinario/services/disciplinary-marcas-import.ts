...
export type MarcasBatchEmailSummary = {
  emailsSent: number;
...
/** Resumen de correos por lote de marcas (historial de importaciones). */
export async function getMarcasBatchEmailSummaries(
  batchIds: string[],
): Promise<Map<string, MarcasBatchEmailSummary>> {
  const summaries = new Map<string, MarcasBatchEmailSummary>();
  ...
  const rows = await prisma.disciplinaryApercibimiento.findMany({
    ...
    select: { importBatchId: true, codigoEmpleado: true, correoEnviadoA: true },
  });

  const byBatch = new Map<string, { sent: number; pendingCodigos: string[] }>();
  const allPendingCodigos = new Set<string>();

  for (const row of rows) {
    if (!row.importBatchId) continue;
    let entry = byBatch.get(row.importBatchId);
    if (!entry) {
      entry = { sent: 0, pendingCodigos: [] };
      byBatch.set(row.importBatchId, entry);
    }
    if (row.correoEnviadoA) {
      entry.sent++;
    } else {
      entry.pendingCodigos.push(row.codigoEmpleado);
      allPendingCodigos.add(row.codigoEmpleado);
    }
  }

  const masters = await getEmployeesForDisciplinaryByCodes([...allPendingCodigos]);

  for (const [batchId, entry] of byBatch) {
    let emailsPendingSendable = 0;
    let emailsPendingNoEmail = 0;
    for (const codigo of entry.pendingCodigos) {
      if (masters.get(codigo)?.email?.trim()) emailsPendingSendable++;
      else emailsPendingNoEmail++;
    }
    summaries.set(batchId, {
      emailsSent: entry.sent,
      emailsPending: entry.pendingCodigos.length,
      emailsPendingSendable,
      emailsPendingNoEmail,
    });
  }
