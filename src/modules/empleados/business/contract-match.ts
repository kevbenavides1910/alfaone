...
/** Puntaje 0–100 de similitud entre contrato RRHH y licitación del sistema. */
export function scoreContractMatch(rrhh: string, licitacion: string): number {
  const a = rrhh.toUpperCase();
  const b = licitacion.toUpperCase();
  if (a === b) return 100;
  ...
  if (prefix >= 8) return 60 + Math.min(20, (prefix / maxLen) * 20);
