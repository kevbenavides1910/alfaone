/** Variables de entorno opcionales (rutas por defecto). Las credenciales SMB se configuran en la UI. */
export const FINGER_ENV = {
  attSmbShare: () =>
    process.env.ATT2016_SMB_SHARE?.trim() || "//10.1.1.3/DB-Biometrico",
  attDatabaseName: () =>
    process.env.ATT2016_DATABASE_FILE?.trim() ||
    process.env.ATT2016_DATABASE_NAME?.trim() ||
    "ATT2016.MDB",
