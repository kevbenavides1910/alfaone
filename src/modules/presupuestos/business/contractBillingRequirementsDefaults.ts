...
export const BASE_DEFAULT_BILLING_REQUIREMENTS = [
  "CCSS certificación",
  "INS",
  "Pólizas",
  "RT",
  ...
  "Planillas CCSS",
  "Planilla INS",
  "Trámite a través de correo",
] as const;

export const SICERE_BILLING_REQUIREMENT = "SICERE";
...
export function getDefaultBillingRequirementsForContract(
  ...
  const list: string[] = [...BASE_DEFAULT_BILLING_REQUIREMENTS];
...
export async function ensureDefaultBillingRequirements(
  ...
  const missing = desired.filter((d) => !existingKeys.has(normalizeRequirementKey(d)));

  if (missing.length === 0) return { added: [] };

  await db.contractBillingRequirement.createMany({
    data: missing.map((description, index) => ({
      contractId,
      description,
  ...
  if (options?.syncOpenFacturas !== false && "facturaMensual" in db) {
    await syncOpenFacturaRequisitosForContractLocal(
      db as BillingDb,
      contractId
    );
  }
