...
import { canViewAllContractCompanies } from "@/modules/core/permissions";
...
export function buildContractListWhere(
  ...
  const restrictToUserCompany = Boolean(
    session.user?.company && !canViewAllContractCompanies(session),
  );
